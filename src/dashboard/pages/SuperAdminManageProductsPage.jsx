...
export function SuperAdminManageProductsPage() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['superadmin', 'manage-products', q, status, page],
    queryFn: async () => {
  ...
  const { data: suggestData } = useQuery({
    queryKey: ['superadmin', 'manage-products-suggest', qInput],
  ...
  const del = useMutation({
    ...
    onSuccess: async () => {
      ...
      await qc.invalidateQueries({ queryKey: ['superadmin', 'manage-products'] })
    },
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <div className="mt-1 text-sm text-[hsl(var(--muted-fg))]">Full control: edit, delete, and manage statuses.</div>
        </div>
        <Link
          ...
          <div key={p._id} className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]/60 p-4 backdrop-blur">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-1 text-xs text-[hsl(var(--muted-fg))]">Created by: {p.createdBy?.name || ''}</div>
              </div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <Link
                to={`/superadmin/products/${p._id}/edit`}
