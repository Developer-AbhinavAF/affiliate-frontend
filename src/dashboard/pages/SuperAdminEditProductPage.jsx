...
export function SuperAdminEditProductPage() {
  const { id } = useParams()
  const nav = useNavigate()

  const { data, isLoading, isError } = useQuery({
    queryKey: ['superadmin', 'product', id],
    queryFn: async () => {
  ...
  return (
    <div className="rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]/60 p-5 shadow-sm backdrop-blur">
      ...
      <ProductForm mode="edit" initial={data} onCancel={() => nav(-1)} onSuccess={() => nav('/superadmin/manage-products')} />
    </div>
  )
}
